...
#./qtl_wing_shape_lme_cluster.r & #Better not to put an out file because it will be huge (lme spits a lot of messages during all the perms)
...
scanone.lm.perm <-
	function(genotypes, phenotype,cross_type,n.perm=10,sex,csize,sex.perms=TRUE,thresholds=c(0.1,0.05,0.01)){				
		result <- NULL
		...
		perm.list<-foreach(perm.it=1:n.perm) %dopar%  {
			...
			csize.randomised<-csize[rand_index]
			cross_type.randomised<-cross_type[rand_index]
			library(lme4)
			if (sex.perms==FALSE){
				lod_null<-logLik(lmer(pheno.randomised ~ csize.randomised + (1|cross_type.randomised),REML=FALSE))								
				lod <- apply(genotypes, 1, function(a) (logLik(lmer(pheno.randomised+ ~ a + csize.randomised + (1|cross_type.randomised),REML=FALSE))-lod_null)/log(10))
			}else{
				lod_null_sex<-logLik(lmer(log(pheno.randomised+ 1) ~ csize.randomised + (1|sex.randomised) + (1|cross_type.randomised),REML=FALSE))
				lod <- apply(genotypes, 1, function(a) (logLik(lmer(log(pheno.randomised+1) ~ a + csize.randomised + (1|sex.randomised) + (1|cross_type.randomised),REML=FALSE))-lod_null_sex)/log(10))
			}			
			max(lod)
		}
		stopImplicitCluster()
...
wingshape_custom<-read.csv(file="qtl_dat_wing_shape_lme.csv",header=FALSE)
#remove sex chromosome but first create dummy scanone for it (see below)
# dummy_sex<-wingshape_custom[which(wingshape_custom$V2=="X"),1:3]
# dummy_sex$lod<-rep(0,nrow(dummy_sex))
# rownames(dummy_sex)<-dummy_sex[,1]
# dummy_sex[,1]<-NULL
# colnames(dummy_sex)<-c("chr","pos","lod")
# wingshape_custom<-wingshape_custom[which(wingshape_custom$V2!="X"),]





FW_rotation_rows<-c(12:47)
HW_rotation_rows<-c(84:109)
...
n.perm<-1000
chom_lengths<-c()
for (i in unique(wingshape_custom$V2[row_start_genotypes:nrow(wingshape_custom)])){
	chom_lengths<-append(chom_lengths,max(wingshape_custom[wingshape_custom$V2==i,]$V3))
}
...
write(header,file="plots/wingshape_custom_cross_type_1000_perms/sig_thresholds.txt",append=TRUE)


sig_thresholds<-c()
for (i in FW_rotation_rows){
	phenotype_row<-i
	pheno_name<-print(wingshape_custom[phenotype_row,1])
	png(filename = paste("plots/wingshape_custom_cross_type_1000_perms/",pheno_name,".png",sep=""), width = 1000, height = 300)
	par(mfrow=c(1,1))
	#run standard r/qtl
	#run custom scripts
	...
	csize<-as.numeric(paste(pedigree[10,4:ncol(pedigree)]))
	sex<-as.numeric(paste(pedigree[5,4:ncol(pedigree)]))		
	lod_null<-logLik(lmer(phenotype ~ csize + (1|cross_type),REML=FALSE))
	lod_null_sex<-logLik(lmer(phenotype ~ csize + (1|sex) + (1|cross_type),REML=FALSE)) 
	lod_auto <- apply(genotypes_autosomes, 1, function(a) (logLik(lmer(phenotype ~ a + csize + (1|cross_type),REML=FALSE))-lod_null)/log(10))		
	lod_sex <- apply(genotypes_sex, 1, function(a) (logLik(lmer(phenotype ~ a + csize + (1|sex) + (1|cross_type),REML=FALSE))-lod_null_sex)/log(10))
	...
	threshold_autosomes<-scanone.lm.perm(genotypes=genotypes_autosomes,phenotype=phenotype,cross_type=cross_type,n.perm=n.perm,sex=sex,csize=csize,sex.perms=FALSE,thresholds=c(LOD_A_0.1,LOD_A_0.05,LOD_A_0.01))
	threshold_sex<-scanone.lm.perm(genotypes_sex,phenotype=phenotype,cross_type=cross_type,n.perm=n.permX,sex=sex,csize=csize,sex.perms=TRUE,thresholds=c(LOD_X_0.1,LOD_X_0.05,LOD_X_0.01))
	...
	write(sig_thresholds2,file="plots/wingshape_custom_cross_type_1000_perms/sig_thresholds.txt",append=TRUE)
}      
...
for (i in HW_rotation_rows){
	...
	cross_type<-as.factor(paste(pedigree[7,4:ncol(pedigree)]))	
	families<-as.factor(paste(pedigree[1,4:ncol(pedigree)]))
	csize<-as.numeric(paste(pedigree[11,4:ncol(pedigree)]))
	sex<-as.numeric(paste(pedigree[5,4:ncol(pedigree)]))			
	lod_null<-logLik(lmer(phenotype ~ csize + (1|cross_type),REML=FALSE))
	lod_null_sex<-logLik(lmer(phenotype ~ csize + (1|sex) + (1|cross_type),REML=FALSE)) 
	lod_auto <- apply(genotypes_autosomes, 1, function(a) (logLik(lmer(phenotype ~ a + csize + (1|cross_type),REML=FALSE))-lod_null)/log(10))		
...
write.csv(sig_thresholds,"plots/wingshape_custom_cross_type_1000_perms/sig_thresholds.csv")
