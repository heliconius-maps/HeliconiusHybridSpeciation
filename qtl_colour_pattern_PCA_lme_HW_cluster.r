...
#./qtl_wing_shape_lme_cluster_rotations.r & #Better not to put an out file because it will be huge (lme spits a lot of messages during all the perms)
...
qtl_dat<-read.csv(file="qtl_dat_NR_all_HWs_rm_bg_aligned_B4_58_white_95%_unconst_PCA_FINAL.csv",header=FALSE)

#this is every PC until they stop explaining >0.05% of the variance. 
phenotype_rows<-c(17:51)
...
write(header,file="results/colour_pattern_PCA/NR_all_HWs_rm_bg_aligned_B4_58_white_95_unconst_PCA_FINAL_lme/sig_thresholds_custom_lme_colour_PCA_HW.txt",append=TRUE)

sig_thresholds<-c()
for (i in phenotype_rows){
	phenotype_row<-i
	pheno_name<-print(qtl_dat[phenotype_row,1])
	png(filename = paste("results/colour_pattern_PCA/NR_all_HWs_rm_bg_aligned_B4_58_white_95_unconst_PCA_FINAL_lme/",pheno_name,".png",sep=""), width = 1000, height = 300)
	par(mfrow=c(1,1))
	#run standard r/qtl
	#run custom scripts
	...
	write(sig_thresholds2,file="results/colour_pattern_PCA/NR_all_HWs_rm_bg_aligned_B4_58_white_95_unconst_PCA_FINAL_lme/sig_thresholds_custom_lme_colour_PCA_HW.txt",append=TRUE)
}      

     
colnames(sig_thresholds)<-c("phenotype","5%_threshold_auto","5%_threshold_sex")
write.csv(sig_thresholds,"results/colour_pattern_PCA/colour_pattern_PCA/NR_all_HWs_rm_bg_aligned_B4_58_white_95_unconst_PCA_FINAL_lme/sig_thresholds_custom_lme_colour_PCA_HW.csv")
